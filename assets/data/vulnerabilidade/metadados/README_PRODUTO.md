...
| `grade_200m_na_bacia` | 36.723 células da Grade Estatística IBGE 2022 (200 m urbano / 1 km rural) dentro da bacia | A grade estatística oferece visualização regular; ela não substitui o setor censitário e possui apenas população e domicílios do produto IBGE. O ID distingue `200M` (urbano) e `1KME` (rural); `area_pct_bacia` registra a fração geométrica. |
