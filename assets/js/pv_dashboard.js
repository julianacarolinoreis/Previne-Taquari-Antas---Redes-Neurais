(function(){
  ...
  function renderBars(){
    ...
    node.innerHTML=hs.map(h=>{const v=rainFor(h),ratio=v==null?0:Math.max(3,Math.min(100,(v/max)*100));const cls=v!=null&&v>=80?'high':v!=null&&v>=40?'warn':'';const detail=`${labelHours(h.hours)} · ${mm(v)}${h.rain!=null&&h.basin!=null&&h.rain!==h.basin?` · ponto ${mm(h.rain)}`:''}`;return `<div class="pv-bar-col" data-tip="${detail}${h.prob!=null?` · estimativa experimental ${pct.format(h.prob)}%`:''}"><span class="pv-bar-value">${mm(v)}</span><span class="pv-bar-track"><span class="pv-bar-fill ${cls}" style="height:${ratio}%"></span></span><span class="pv-bar-label">${labelHours(h.hours)}</span><span class="pv-bar-caption">${v==null?'sem chuva':'acumulada'}</span></div>`}).join('');
    setText('#pv-chart-title','Chuva acumulada por horizonte');setText('#pv-chart-unit','mm');setText('#pv-chart-subtitle',station==='mucum'?'Barras: proxy do feed local; o ponto é mostrado no detalhe.':'Barras: média prevista no recorte da bacia.');setText('#pv-chart-note','Volume previsto não equivale sozinho a transbordamento: importam localização da chuva, nível do rio, solo e tempo de propagação.');
  }
  function renderSide(){
