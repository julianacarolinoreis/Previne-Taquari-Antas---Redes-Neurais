...
def validar_html(nome: str, esperado: dict[str, str]) -> None:
    ...
    assert "if(mins<=30)return 'telemetria recente'" not in texto, f"{nome}: limiar antigo de telemetria voltou ao HTML"
    assert "Status do feed na publicação:" not in texto, f"{nome}: HTML voltou a misturar status antigo do snapshot com idade atual"
    assert "idadeRobo!=null && idadeRobo>60" not in texto, f"{nome}: HTML tolera robô parado por 1h"
    assert "idadeAna!=null && idadeAna>120" not in texto, f"{nome}: HTML tolera telemetria velha por 2h"
    assert "proxima_base_diagnostico" in texto, f"{nome}: motivo da próxima hora-base não é exibido"
