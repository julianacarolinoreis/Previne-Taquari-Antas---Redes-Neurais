...
class SantaTerezaMdtRefinementTests(unittest.TestCase):
    ...
    def test_hand_generator_uses_flow_direction_not_nearest_river(self) -> None:
        ...
        self.assertIn('terrain_path = args.source_dir / "CLIP_MOSAICO_LIDAR_RS.tif"', script)
        self.assertIn('routing_path = args.source_dir / "FILL_CLIP_MOSAICO_LIDAR_RS.tif"', script)
        self.assertIn('"hand_method": "raw LiDAR elevation relative to D8-connected main-river cell"', script)
        self.assertIn('"hydraulic_hand_method": "maximum raw-LiDAR barrier along D8 path to main river"', script)
        self.assertIn('"fill_usado_como_superficie_inundacao": False', script)
        self.assertNotIn("distance_transform_edt", script)
        self.assertIn("Resampling.mode", script)
        self.assertIn("encoded = np.full(hydraulic_hand.shape, 255", script)
        ...
        self.assertIn('"fonte": "CLIP_MOSAICO_LIDAR_RS.tif"', script)
