...
def main() -> int:
    ...
    assert "https://{s}.tile.openstreetmap.org" not in text, "host OSM antigo ainda publicado"
    assert "https://tile.openstreetmap.org/{z}/{x}/{y}.png" in text, "host OSM canônico ausente"
    assert text.count('referrerPolicy:"strict-origin-when-cross-origin"') >= 2, "Referer OSM não preservado nos dois mapas"
    assert "https://www.openstreetmap.org/copyright" in text and "OpenStreetMap</a> contributors" in text, "atribuição OSM incompleta"
    assert "https://tile.openstreetmap.org https://server.arcgisonline.com" in text, "CSP não permite o host OSM canônico"
