...
LOCAL_DSS = ROOT / "assets" / "data" / "hec_hms_calibration" / "mucum_santa_tereza_raw_rain_events.dss"
OUT_DSS = ROOT / "assets" / "data" / "hec_hms_calibration" / "mucum_santa_tereza_raw_rain_complete_v2.dss"
...
def complete_events(series: dict[str, dict[datetime, tuple[float, int]]]) -> list[str]:
    return [
        event_id for event_id, values in series.items()
        if values and all(right - left == timedelta(hours=1)
                          for left, right in zip(sorted(values), sorted(values)[1:]))
    ]


def write_jython(allowed: list[str]) -> Path:
    ...
    script.write_text(
        ...
        "months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']\n"
        "direct_paths = ['/MUCUM/RAIN_E28_86472000/PRECIP-INC/01Jun2024/1Hour/OBS/']\n"
        ...
        "        date, clock = row['timestamp_label'].split(' ')\n"
...
def main() -> int:
    ...
    write_csv(series)
    allowed = complete_events(series)
    script = write_jython(allowed)
