...
class ArtigoRnaSantaTerezaTests(unittest.TestCase):
    ...
    def test_manuscript_keeps_the_consolidated_claims(self) -> None:
        ...
        self.assertIn("Desempenho do modelo de maior equilíbrio publicado", self.manuscript)
        self.assertNotIn("Desempenho no teste do modelo", self.manuscript)
        self.assertIn("| 32,0 |", self.manuscript)
        self.assertIn("Equilíbrio é o campo publicado na ficha", self.manuscript)
        self.assertNotIn("Identificadores MATLAB:", self.manuscript.split("**Tabela 1")[1].split("Em 2 h")[0])
