...
def read_event(event_id: str, manifest_event: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "selection_note": manifest_event.get("selection_note", ""),
...
const fmt0=new Intl.NumberFormat("pt-BR",{maximumFractionDigits:0}), fmt1=new Intl.NumberFormat("pt-BR",{minimumFractionDigits:1,maximumFractionDigits:1}), fmtPct=new Intl.NumberFormat("pt-BR",{minimumFractionDigits:3,maximumFractionDigits:3});
...
function metricText(){const m=current().metrics;return `Pico observado ${fmt1.format(m.observed_peak_m3s)} m³/s; HEC-HMS ${fmt1.format(m.simulated_peak_m3s)} m³/s; erro ${fmtPct.format(m.peak_relative_error*100)}%; atraso ${m.peak_lag_hours>0?"+":""}${fmt1.format(m.peak_lag_hours)} h; NSE ${fmt1.format(m.nse)}.`}
...
function render(){const e=current(),m=e.metrics;$('eventTitle').textContent=e.id;$('eventPeriod').textContent=e.period;$('eventStatus').textContent=e.status==='fechado'?'replay fechado · horas pareadas':'diagnóstico · ainda não promovido';$('chartTitle').textContent=`${e.id} · observado × HEC-HMS`;$('obsPeak').textContent=fmt1.format(m.observed_peak_m3s);$('simPeak').textContent=fmt1.format(m.simulated_peak_m3s);$('peakError').textContent=fmtPct.format(m.peak_relative_error*100)+'%';$('peakLag').textContent=(m.peak_lag_hours>0?'+':'')+fmt1.format(m.peak_lag_hours)+' h';$('nse').textContent=fmt1.format(m.nse);$('pairs').textContent=fmt0.format(m.pairs);$('metricsLink').href=e.metrics_href;$('seriesLink').href=e.series_href;$('selectionNote').textContent=e.selection_note||'';$('peakDates').textContent=`Pico observado e HEC-HMS: ${peakDate(e,1)} · ${peakDate(e,2)}`;$('srSummary').textContent=metricText();draw()}
