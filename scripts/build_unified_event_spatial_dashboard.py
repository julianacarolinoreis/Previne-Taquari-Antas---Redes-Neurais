...
const fmt0=new Intl.NumberFormat("pt-BR",{maximumFractionDigits:0}),fmt1=new Intl.NumberFormat("pt-BR",{minimumFractionDigits:1,maximumFractionDigits:1}),fmt2=new Intl.NumberFormat("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),fmtPct=new Intl.NumberFormat("pt-BR",{minimumFractionDigits:3,maximumFractionDigits:3});
const dateFmt=new Intl.DateTimeFormat("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"});
const state={key:"mucum-E24",start:0,end:0,hover:null,dragStart:null,dragCurrent:null,city:"all",selectedCell:null,selectedService:null,map:{scale:1,tx:0,ty:0,drag:false,lastX:0,lastY:0}};
...
function renderSummary(){const mucum=DATA.events.filter(e=>e.city_key==="mucum"),santa=DATA.events.filter(e=>e.city_key==="santa_tereza");$("totalEvents").textContent=fmt0.format(DATA.events.length);$("mucumEvents").textContent=fmt0.format(mucum.length);$("santaEvents").textContent=fmt0.format(santa.length);$("independentEvents").textContent=fmt0.format(DATA.events.filter(e=>e.status.includes("independente")).length);renderCalibration()}
function renderCalibration(){const c=DATA.calibration;if(!c)return;const zone=c.three_zone_events||[],target=c.target_rain_events||[],best=c.two_station_best||{};$("calibrationStatus").textContent=`${c.overall_status}. A rede de três zonas só tem cobertura completa em ${zone.length} evento${zone.length===1?"":"s"}; isso ainda não permite validar generalização.`;$("threeZoneCount").textContent=fmt0.format(zone.length);$("threeZoneDetail").textContent=zone.length?zone.join(", "):"nenhum evento fechado";$("targetRainCount").textContent=fmt0.format(target.length);$("targetRainDetail").textContent=target.length?`E${target.join(", E")}`:"nenhum evento fechado";$("twoStationScore").textContent=best.mean_nse==null?"—":fmt2.format(best.mean_nse);$("twoStationDetail").textContent=best.mean_abs_peak_lag_hours==null?"sem busca concluída":`NSE médio · atraso médio ${fmt1.format(best.mean_abs_peak_lag_hours)} h · erro de pico ${fmtPct.format(best.mean_peak_relative_error*100)}%`}
function renderEvent(){const e=current(),m=e.metrics;$('eventTitle').textContent=`${e.municipality} · ${e.id}`;$('eventSubtitle').textContent=`${e.period} · ${e.variable} · ${e.series.length} pontos`;const st=$("eventStatus");st.textContent=niceStatus(e);st.className=statusClass(e);$('obsPeak').textContent=e.unit==="cm"?fmt1.format(m.observed_peak):fmt1.format(m.observed_peak);$('modelPeak').textContent=fmt1.format(m.model_peak);$('unitObs').textContent=e.unit;$('unitModel').textContent=e.unit;$('peakError').textContent=m.peak_error==null?"—":fmtPct.format(m.peak_error*100)+"%";$('peakLag').textContent=m.lag==null?"—":(m.lag>0?"+":"")+fmt1.format(m.lag)+" h";$('pairs').textContent=fmt0.format(m.pairs||e.series.length);if(m.nse!=null){$('fitMetric').textContent=fmt2.format(m.nse);$('fitLabel').textContent="NSE"}else{$('fitMetric').textContent=m.mae==null?"—":fmt1.format(m.mae);$('fitLabel').textContent="MAE · "+e.unit}$('chartTitle').textContent=`${e.id} · observado × ${e.model}`;$('modelLegend').textContent=e.model;$('peakDates').textContent=`Picos: observado ${peakDate(e,1)} · modelo ${peakDate(e,2)}`;$('reading').textContent=e.city_key==="mucum"?(e.status==="fechado"?"Replay fechado no pacote de pesquisa. Compare forma, pico e horário; o conjunto ainda usa uma bacia agregada e não uma delineação hidrológica completa derivada do MDT.":"Diagnóstico preservado para mostrar o que ainda precisa melhorar: há lacuna ou subestimação no evento e ele não foi promovido."):e.status.includes("independente")?"Teste independente da rotação RNA: a comparação está separada do treino e serve para avaliar generalização do replay de nível.":"Replay de nível da RNA: útil para estudar forma, pico e atraso, mas não é calibração HEC-HMS de vazão.";$('metricsLink').href=hrefFor(e.metrics_source);$('seriesLink').href=hrefFor(e.series_source);$('manifestLink').href=e.city_key==="mucum"?"../assets/data/mucum_eventwise_replay_calibrated/eventwise_manifest.json":"../assets/data/santa_tereza_eventwise_replay_rna_2h/eventwise_manifest.json";state.start=0;state.end=Math.max(0,e.series.length-1);renderMap();renderEventsTable();draw()}
...
function showPoint(p,index=null){state.selectedService=index;document.querySelectorAll(".service").forEach((el,i)=>el.classList.toggle("selected",i===index));$("cellInfo").innerHTML=`<strong>${index==null?"Ponto de apoio":`Ponto ${index+1}`} · ${p.kind}: ${p.name}</strong><p>Ponto cadastrado na camada <b>${p.source.split("/").pop()}</b>. Clique em outra célula ou ponto para continuar a exploração. A presença no mapa não confirma abertura, capacidade, acessibilidade, rota ou disponibilidade durante um evento.</p>`}
function renderEventsTable(){const rows=$("eventRows");rows.innerHTML="";const list=state.city==="all"?DATA.events:DATA.events.filter(e=>e.city_key===state.city);$("tableScope").textContent=`${list.length} evento${list.length===1?"":"s"}`;for(const e of list){const tr=document.createElement("tr");if(e.key===state.key)tr.className="selected";tr.addEventListener("click",()=>{state.key=e.key;$("eventSelect").value=e.key;renderEvent()});const m=e.metrics;tr.innerHTML=`<td><button class="event-button" type="button">${e.municipality} · ${e.id}</button></td><td>${e.municipality}</td><td>${e.model}</td><td>${e.period.split(" → ")[0]}</td><td>${m.observed_peak==null?"—":fmt1.format(m.observed_peak)} ${e.unit}</td><td>${m.model_peak==null?"—":fmt1.format(m.model_peak)} ${e.unit}</td><td>${m.peak_error==null?"—":fmtPct.format(m.peak_error*100)+"%"}</td><td><span class="tiny-pill ${e.status.includes("independente")?"test":e.status==="fechado"?"":"warn"}">${e.status}</span></td>`;rows.append(tr)}}
...
(function(){
  ...
  function syncSelectionNote(){const e=eventByKey(state.key);note.textContent=e.selection_note||"";note.style.display=e.selection_note?"block":"none";}
  const baseRenderEvent=renderEvent;
  renderEvent=function(){baseRenderEvent();syncSelectionNote();};
  syncSelectionNote();
  document.getElementById("eventSelect").addEventListener("change",()=>setTimeout(syncSelectionNote,0));
  document.getElementById("citySelect").addEventListener("change",()=>setTimeout(syncSelectionNote,0));
})();
</script></body></html>''',
)
