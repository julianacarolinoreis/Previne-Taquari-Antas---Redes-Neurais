...
def spatial_data() -> dict[str, object]:
    ...
    return {
        "mucum": {
            ...
            "bounds": mucum_bounds,
            "crs": "EPSG:4326",
            "level_min": int(mucum["published_level_range_m"][0]),
            "level_max": int(mucum["published_level_range_m"][1]),
            "contours": selected_contours(
                ROOT / "assets/data/mucum_inundacao/contornos_mancha.json",
                set(range(int(mucum["published_level_range_m"][0]), int(mucum["published_level_range_m"][1]) + 1)),
            ),
            "grid": mucum_grid,
        ...
        "santa_tereza": {
            ...
            "level_max": int(santa["published_level_range_m"][1]),
            "contours": selected_contours(
                ROOT / "assets/data/santa_tereza_inundacao/contornos_mancha.json",
                set(range(int(santa["published_level_range_m"][0]), int(santa["published_level_range_m"][1]) + 1)),
            ),
            "grid": santa_grid,
...
def calibration_snapshot() -> dict[str, object]:
    ...
    common_recent = status.get("diagnostic_searches", {}).get("common_parameters_recent", {})
    common_best = common_recent.get("best_candidate") or {}
    ...
    return {
        ...
        "common_search_recent": {
            "candidate_count": common_recent.get("candidate_count", 0),
            "successful": common_recent.get("successful_candidates", 0),
            "events": common_best.get("events"),
            "mean_nse": common_best.get("mean_nse"),
            "mean_abs_peak_lag_hours": common_best.get("mean_abs_peak_lag_hours"),
            "mean_peak_relative_error": common_best.get("mean_peak_relative_error"),
            "fitness": common_best.get("fitness"),
            "status": common_recent.get("status"),
            "rainfall_policy": common_recent.get("rainfall_policy"),
            "artifact": common_recent.get("artifact"),
            "results_artifact": common_recent.get("results_artifact"),
        },
        "links": {
